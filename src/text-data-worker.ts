class TextualDataWorker {
    private static readonly ARTICLE_PATTERN = /Art\.\s+\d+\./g;
    private static readonly HEADS_TO_DEL_PATTERN = /(Titre|Chapitre|Sous-titre|Section|Sous-section) \d?(er|re)*\./
    ...
    processText(input: string): string {
        ...
        let text = input.replace(TextualDataWorker.LEGAL_REFERENCE_PATTERN, '');

        // Process footnotes
        text = text.replace(TextualDataWorker.FOOTNOTE_PATTERN, '$1');
...
const processedText = worker.processText(`CHAPITRE 1er. - Dispositions introductives
